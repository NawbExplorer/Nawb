...
import { useCarlaUIPluginNavigation } from '../../plugin-core/components';
import {
  renderCarlaToReact,
  EM,
  PostBridgeAction,
...
> = observer(function (props) {
  ...
  const { params } = route;
  const [carlaUI, setCarlaUI] = useState<ReactNode>(null);
  const pluginCtx = useContext(PluginContext);

  const pluginNavigation = useCarlaUIPluginNavigation(params.pluginName);
